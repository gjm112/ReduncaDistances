...
for (toothtype in c("LM1","LM2","LM3","UM1","UM2","UM3")){print(toothtype)
  ...
  mds <- cmdscale(ddd)
  dat <- dat %>% bind_rows(data.frame(toothtype = toothtype , class = class, x = mds[,1], y = mds[,2]))
...
dat$species <- dat$class
dat$species[dat$species == "pricei"] <- "T. pricei"
dat$species[dat$species == "scriptus"] <- "T. scriptus"

png("./mds_shape_only.png", res = 300, unit = "in", w = 8, h = 5)
ggplot(aes(x = x, y = y, color = species), data = dat) + geom_point(alpha = 0.75) + theme_bw() + facet_grid(upperlower ~ toothnum)
dev.off()
